...
class Actor(nn.Module):
    ...
    def forward(self, state, his_state, his_len):
        ...
        for layer in self.HFC2: his_input = layer(his_input)                                          
        his_output = torch.sum(his_input, dim = 1) * 0.1
        ...
        for layer in self.FinalFC: final_input = layer(final_input)                     
        action = final_input
...
class Critic(nn.Module):
    ...
    def forward(self, state, action, his_state, his_action, his_len):
        ...
        for layer in self.HS2: hs = layer(hs)
        for layer in self._HS2: _hs = layer(_hs)

        hist_out = torch.sum(hs, dim = 1) * 0.1
        _hist_out = torch.sum(_hs, dim = 1) * 0.1
